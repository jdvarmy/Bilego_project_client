...
class MainSlider extends React.Component {
  state = {update: this.props.globalStore.city};
  ...
  //   if(prevState.update === this.state.update && this.state.update === true){
  //     // todo: сделать обновление слайдера при смене города
  //     this.setState({update: false});
  //     this.props.sliderStore.revapi.revkill();
  //     this.setSlider();
  //   }
  // };

  componentWillUpdate(nextProps, nextState){
    if(nextState.update !== this.props.globalStore.city){
      this.setState({update: this.props.globalStore.city})
      console.log(nextState)
      console.log(this.props.globalStore.city)
      // this.props.sliderStore.revapi.revkill();
      this.setSlider();
    }
  };
  ...
  setSlider = async () => {
    const {sliderStore:{getMainSlides, clear}} = this.props;
    clear();

    await getMainSlides();
    this.renderRevolutionSliderMain();
  };

  renderRevolutionSliderMain = () => {
    const slider = document.getElementById('rev_slider_7_1');

    if(window.$(slider).revolution !== undefined) {
      const revapi = window.$(slider).show().revolution({
        jsFileLocation: "js/",
        sliderLayout: "auto",
      ...
      this.props.sliderStore.setRevapi(revapi);
    }
  };

  render() {
    const {sliderStore:{slides}} = this.props;

    return (
      <rs-module-wrap id="rev_slider_7_1_wrapper" data-alias="clear-cut" data-source="gallery" style={{background: 'transparent', padding: 0}}>
        <rs-module id="rev_slider_7_1" style={{display: 'none'}}>
          <rs-slides>
            {slides.map((slide,k)=>(
              <Slide id={slide.id} key={slide.id} title={slide.title}>
                ...
                <SliderZone id={slide.id} title={slide.image_title} title1={slide.image_title1} title2={slide.image_title2}/>
                <SliderGroup id={slide.id} slidesCnt={slides.length} slideNumber={k+1}/>
                <SliderLayer id={slide.id} link={slide.ticket_link} />
              </Slide>
            ))}
          </rs-slides>
...
function SliderLayer(props){
  ...
  return (
    <rs-layer
      ...
      data-type="text"
      data-xy="x:c,c,c,l;y:b,b,b,t;yo:100px,100px,100px,0;"
    ...
    ><a className="bilego-ticket-view" href={link}>View Project <i className="fa-ticket" style={{marginLeft:'10px'}} /></a>
    </rs-layer>
  );
}